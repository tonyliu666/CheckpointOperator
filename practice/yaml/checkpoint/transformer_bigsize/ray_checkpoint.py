...
#                 }
#                 checkpoint_path = os.path.join("checkpoints", f"checkpoint_{i}.pt")
#                 torch.save(checkpoint, checkpoint_path)
#                 air.session.report({"val_loss": val_loss, "train_loss": train_loss}, checkpoint=Checkpoint.from_directory("checkpoints"))
#                 log.info(f"Saved Best Weights to {checkpoint_path}")

#         log.info(f"Training Loss : {best_train_loss}")
#         log.info(f"Valid Loss : {best_valid_loss}")
    ...
    def train(self):
        ...
        for i in range(self.epoch):
            ...
            if val_loss < best_valid_loss:
                ...
                torch.save(checkpoint, checkpoint_path)
                # air.session.report({"val_loss": val_loss, "train_loss": train_loss}, checkpoint=Checkpoint.from_directory(checkpoint_dir))
                report({"val_loss": val_loss, "train_loss": train_loss}, checkpoint=Checkpoint.from_directory(checkpoint_dir))
                log.info(f"Saved Best Weights to {checkpoint_path}")
